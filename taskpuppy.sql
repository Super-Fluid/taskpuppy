...
create table projects (
  ...
  title                 text                                                            not null,
  details               text                                                            ,
  owner                 integer                         references snap_auth_user(uid)  not null
);
...
create table issues (
  id                    serial                          primary key                     ,
  title                 text                                                            not null,
  creator               integer                         references snap_auth_user(uid)  not null,
  last_updated          timestamp with time zone                                        not null, -- will be the same as dateCreated for new issues
  date_created          timestamp with time zone                                        not null,
  assigned_to           integer                         references snap_auth_user(uid)  , -- null: unassigned
  issue_status          issue_status                                                    not null,
  issue_priority        issue_priority                                                  not null,
  project               integer                         references projects(id)          -- null is to save a draft that is not assigned.. (should be very rare)
);

create table issue_updates (
  id                    serial                          primary key                     ,
  issue                 integer                         references issues(id)           not null,
  author                integer                         references snap_auth_user(uid)  not null,
  ...
  old_assignee          integer                         references snap_auth_user(uid)  , -- null: this update didn't reassign the issue
  date_published        timestamp with time zone                                         -- null: draft
);

create table watches_issue_map (
  watcher               integer                         references snap_auth_user(uid)  not null,
  issue                 integer                         references issues(id)           not null
);